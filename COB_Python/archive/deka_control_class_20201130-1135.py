...
class DekaControl():
    ...
    def __init__(self, direct_aci=False):
        ...
        #wristfe: -3520-3520
        #wristrot: -7680-11200
        self.limits_thumb = self._limit_pos_sensor(self.rest_pos_aci[0],self.min_pos_aci[0],self.max_pos_aci[0],0,5760)
        self.limits_ind = self._limit_pos_sensor(self.rest_pos_aci[1],self.min_pos_aci[1],self.max_pos_aci[1],0,5760)
        self.limits_mrp = self._limit_pos_sensor(self.rest_pos_aci[2],self.min_pos_aci[2],self.max_pos_aci[2],0,5760)
        self.limits_thumbint = self._limit_pos_sensor(self.rest_pos_aci[3],self.min_pos_aci[3],self.max_pos_aci[3],0,4800)
        self.limits_wristfe = (0,3520,3520)
        self.limits_wristrot = (0,7680,11200)
    ...
    def _limit_pos_sensor(self, rest_aci, min_aci, max_aci, min_sensor, max_sensor):
        # normalize in terms of ACI motor commands
        rest_sensor = (rest_aci-min_aci) / (max_aci-min_aci) * (max_sensor-min_sensor) + min_sensor
        
        return rest_sensor, min_sensor, max_sensor
    ...
    def norm_pos_sensor(self, sensor, limits):
        # normalize [-1, 1]
        rest = limits[0]
        negmax = limits[1]
        posmax = limits[2]
        if sensor >= rest:
            mapped_sensor = (sensor-rest)/(posmax-rest) 
        else:
            mapped_sensor = (sensor-negmax)/(rest-negmax)-1 
        ...
        while True:
            ...
            try: #Receiving message from LUKE arm
                ...
                if 'sync' in aci_recvd:
                    ...
                    self.sensors[13] = self.norm_pos_wrist(aci_recvd['wrist_pron'],self.limits_wristrot)
                    self.sensors[14] = self.norm_pos_wrist(-aci_recvd['wrist_flex'],self.limits_wristfe)
                    self.sensors[15] = self.norm_pos_sensor(aci_recvd['index_finger'],self.limits_ind)
                    self.sensors[16] = self.norm_pos_sensor(aci_recvd['mrp'],self.limits_mrp)
                elif 'thumb_pitch' in aci_recvd:
                    self.sensors[17] = self.norm_pos_sensor(aci_recvd['thumb_pitch'],self.limits_thumbint)
                    self.sensors[18] = self.norm_pos_sensor(aci_recvd['thumb_yaw'],self.limits_thumb)
